...
const char* ssid = "Silvano";
const char* password = "14069701";
const char* mqttServer = "test.mosquitto.org";
const int mqttPort = 1883;
const char* mqttTopic = "DIP/lock";
...
{
  ESP_LOGD(TAG, "ZAPOČNI DODAVANJE KORISNIKA");
  int left_sample_face = enroll_face_id_to_flash_with_name(face_list, new_id, st_name.enroll_name);
  ESP_LOGD(TAG, "DODAVANJE ID %s LICA: Sample %d",
           st_name.enroll_name,
      ...
      if (out_res.net_boxes)
      {
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_ENROLL)
          {
            ...
            sprintf(enrolling_message, "PRIMJERAK %d %s", ENROLL_CONFIRM_TIMES - left_sample_face, st_name.enroll_name);
            client.send(enrolling_message);
            if (left_sample_face == 0)
            {
              ESP_LOGI(TAG, "PRIMJERAK LICA ID: %s", st_face_list.tail->id_name);
              g_state = START_STREAM;
              char captured_message[64];
              sprintf(captured_message, "SPREMLJENO LICE %s", st_face_list.tail->id_name);
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              sprintf(recognised_message, "DOBRO DOŠLI %s", f->id_name);
            ...
            {
              attempt++;
              client.send("LICE NIJE PREPOZNATO");
              delay(1000);
              if(attempt==3){
                digitalWrite(LED_BUILTIN, LOW);
                sendMessage("DIP/lock", "Lice nepoznato");
                client.send("ZAKLJUČANO");
                lock = true;
                esp_camera_fb_return(fb);
                break;
...
void sendMessage(const char* topic, const char* payload) {
  if (client2.connected()) {
    bool sent = client2.publish(topic, payload);
    if (sent) {
